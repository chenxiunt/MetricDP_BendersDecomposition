function agent = subProblem(agent, env_parameters, Z) 
    ...
    options = optimoptions('linprog','Algorithm','dual-simplex','Preprocess', 'none', 'Display', 'off');

    % options = optimoptions('linprog','Algorithm','interior-point', 'Preprocess', 'none');
    ...
    if exitflag == -3
        % extremeRays = doubleDescriptionMethod(A', c);
        agent.isunbounded = 1; 
        agent.upperbound = 9;
        % options = optimoptions('linprog','Algorithm','interior-point', 'Preprocess', 'none');
        options = optimoptions('linprog','Algorithm','dual-simplex','Preprocess', 'none', 'Display', 'off');
